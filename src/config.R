...
config <- function(dirs=T, cleaning=F, insitu=F, exsitu=F, modeling=F, premodeling=F) {
  ...
  if (dirs) {
    ...
    gap_dir <<- paste0(root, "/gap_analysis_201803");if(!file.exists(gap_dir)){dir.create(gap_dir)}
    par_dir <<- paste0(root, "/parameters_201803");if(!file.exists(par_dir)){dir.create(par_dir)}
