  ...
  if (file.exists(paste(occ_dir,"/no_sea/",species,".csv",sep=""))) {
    ...
    rm(xy_data_na)
      ...
      if(!file.exists(paste0(crossValDir, "/modeling_results.", species, ".RDS"))){
        ...
        #bck_data_bio <- cbind(bck_data, raster::extract(rst_vx, sp::SpatialPoints(bck_data[,c("lon", "lat")])))
        ...
        #xy_data_bio <- cbind(xy_data, raster::extract(rst_vx, sp::SpatialPoints(xy_data[,c("lon", "lat")])))
